...
namespace UE4Math
{
	...
	inline VectorRegister VectorNormalizeAccurate(const VectorRegister& Vector)
	{
	...
	inline VectorRegister VectorNormalizeSafe(const VectorRegister& Vector, const VectorRegister& DefaultValue)
	{
		const VectorRegister SquareSum = VectorDot4(Vector, Vector);
	 ...
	 */
	inline uint32 VectorAnyLesserThan(VectorRegister Vec1, VectorRegister Vec2)
	{
	 ...
	 * @return				Non-zero integer if (Vec1.x > Vec2.x) && (Vec1.y > Vec2.y) && (Vec1.z > Vec2.z) && (Vec1.w > Vec2.w)
	 */
	inline uint32 VectorAllGreaterThan(VectorRegister Vec1, VectorRegister Vec2)
	{
		return !VectorAnyGreaterThan(Vec2, Vec1);
	}
	 ...
	 * @return				Non-zero integer if (Vec1.x < Vec2.x) && (Vec1.y < Vec2.y) && (Vec1.z < Vec2.z) && (Vec1.w < Vec2.w)
	 */
	inline uint32 VectorAllLesserThan(VectorRegister Vec1, VectorRegister Vec2)
	{
	...
	template<> inline VectorRegister FGenericPlatformMath::Min(const VectorRegister A, const VectorRegister B)
	{
		return VectorMin(A, B);
	}

	/** Returns the larger of the two values (operates on each component individually) */
	template<> inline VectorRegister FGenericPlatformMath::Max(const VectorRegister A, const VectorRegister B)
	{
		return VectorMax(A, B);
	}

	// Specialization of Lerp template that works with vector registers
	template<> inline VectorRegister FMath::Lerp(const VectorRegister& A, const VectorRegister& B, const VectorRegister& Alpha)
	{
	...
	inline VectorRegister VectorAccumulateQuaternionShortestPath(const VectorRegister& A, const VectorRegister& B)
	{
		// Blend rotation
	...
	inline VectorRegister VectorNormalizeQuaternion(const VectorRegister& UnnormalizedQuat)
	{
		return VectorNormalizeSafe(UnnormalizedQuat, GlobalVectorConstants::Float0001);
	}

	// Normalize Rotator
	inline VectorRegister VectorNormalizeRotator(const VectorRegister& UnnormalizedRotator)
	{
	 ...
	 */
	inline VectorRegister VectorLerpQuat(const VectorRegister& A, const VectorRegister& B, const VectorRegister& Alpha)
	{
		// Blend rotation
		//     To ensure the 'shortest route', we make sure the dot product between the both rotations is positive.
		//     const float Bias = (|A.B| >= 0 ? 1 : -1)
	 ...
	 * Result is NOT normalized.
	 */
	inline VectorRegister VectorBiLerpQuat(const VectorRegister& P00, const VectorRegister& P10, const VectorRegister& P01, const VectorRegister& P11, const VectorRegister& FracX, const VectorRegister& FracY)
	{
	 ...
	 */
	inline VectorRegister VectorQuaternionInverse(const VectorRegister& NormalizedQuat)
	{
	 ...
	 */
	inline VectorRegister VectorQuaternionInverseRotateVector(const VectorRegister& Quat, const VectorRegister& VectorW0)
	{
		// Q.Inverse * V * Q
		//const VectorRegister InverseRotation = VectorQuaternionInverse(Quat);
	...
	*/
	inline void VectorQuaternionRotateVectorPtr(void* __restrict Result, const void* __restrict Quat, const void* __restrict VectorW0)
	{
	...
	* @param VectorW0	Pointer to the vector (must not be the destination). W component must be zero.
	*/
	inline void VectorQuaternionInverseRotateVectorPtr(void* __restrict Result, const void* __restrict Quat, const void* __restrict VectorW0)
	{
