...
namespace UE4Math
{
	...
	template< class T >
	inline FInterpCurvePoint<T>::FInterpCurvePoint(const float In, const T& Out)
		: InVal(In)
		, OutVal(Out)
	{
	...
	template< class T >
	inline FInterpCurvePoint<T>::FInterpCurvePoint(const float In, const T& Out, const T& InArriveTangent, const T& InLeaveTangent, const EInterpCurveMode InInterpMode)
		: InVal(In)
		, OutVal(Out)
	...
	template< class T >
	inline bool FInterpCurvePoint<T>::IsCurveKey() const
	{
