 ...
 */
	...
	public:
		...
		*/
		 ...
		 */
		static float EvaluateBezier(const FVector* ControlPoints, int32 NumPoints, std::vector<FVector>& OutPoints);
		 ...
		 */
		static void GenerateClusterCenters(std::vector<FVector>& Clusters, const std::vector<FVector>& Points, int32 NumIterations, int32 NumConnectionsToBeValid);
...
//#endif
	...
	inline FIntVector::FIntVector(FVector InVector)
	...
	inline float& FVector::Component(int32 Index)
	{
		return (&X)[Index];
	}

	inline float FVector::Component(int32 Index) const
	{
		return (&X)[Index];
	}

	inline float FVector::GetComponentForAxis(EAxis::Type Axis) const
	{
		switch (Axis)
		{
		case EAxis::X:
	...
	inline void FVector::SetComponentForAxis(EAxis::Type Axis, float Component)
	{
		switch (Axis)
		{
		case EAxis::X:
	...
	inline std::string FVector::ToString() const
	{
		char stemp[200] = "";
		snprintf(stemp, 200, "X=%3.3f Y=%3.3f Z=%3.3f", X, Y, Z);
		return std::string(stemp);
		//return FString::Printf(TEXT("X=%3.3f Y=%3.3f Z=%3.3f"), X, Y, Z);
	}


	inline std::string FVector::ToCompactString() const
	{
		if (IsNearlyZero())
		{
			return "V(0)";
		...
		std::string ReturnString("V(");
		bool bIsEmptyString = true;
		if (!FMath::IsNearlyZero(X))
		{
			char stemp[50] = "";
			snprintf(stemp, 50, "X=%.2f", X);
...
#if PLATFORM_VECTOR_CUBIC_INTERP_SSE
	template<>
	inline_DEBUGGABLE FVector FMath::CubicInterp(const FVector& P0, const FVector& T0, const FVector& P1, const FVector& T1, const float& A)
	{
